...
const sendEmail = async (to, subject, html) => {
    ...
    try {
        await transporter.sendMail({
            from: `"HealthBridge Healthcare" <${process.env.EMAIL_USER}>`,
            to,
            subject,
            html,
        });
...
const sendReportUploadedEmail = (patientEmail, patientName, reportType) => {
    return sendEmail(
        patientEmail,
        "New Medical Report Uploaded — HealthBridge Healthcare",
        `<div style="font-family: 'Segoe UI', sans-serif; max-width: 520px; margin: auto; padding: 30px; border: 1px solid #e5e7eb; border-radius: 12px;">
            ...
            <p style="margin-top: 24px; font-size: 12px; color: #9ca3af;">— HealthBridge Healthcare Team</p>
        </div>`
    );
};
...
const sendPrescriptionAddedEmail = (patientEmail, patientName, doctorName) => {
    return sendEmail(
        patientEmail,
        "New Prescription Added — HealthBridge Healthcare",
        `<div style="font-family: 'Segoe UI', sans-serif; max-width: 520px; margin: auto; padding: 30px; border: 1px solid #e5e7eb; border-radius: 12px;">
...
const sendAppointmentConfirmedEmail = (patientEmail, patientName, doctorName, date, timeSlot) => {
    return sendEmail(
        patientEmail,
        "Appointment Confirmed — HealthBridge Healthcare",
        `<div style="font-family: 'Segoe UI', sans-serif; max-width: 520px; margin: auto; padding: 30px; border: 1px solid #e5e7eb; border-radius: 12px;">
